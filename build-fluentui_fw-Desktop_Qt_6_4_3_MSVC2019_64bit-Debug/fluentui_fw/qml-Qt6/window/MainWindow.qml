...
FluWindow{
    ...
    FluContentDialog{
        ...
        onNegativeClicked: {
            system_tray.showMessage("友情提示","MIL报表生成软件已隐藏至托盘,点击托盘可再次激活窗口");
    ...
    FluTour{
        ...
        steps:{
            var data = []
            if(!window.useSystemAppBar){
                data.push({title:"主题切换",description: "点击这里可以切换主题模式.",target:()=>appBar.darkButton()})
            }
            data.push({title:"隐藏功能",description: "多点几下可以触发隐藏功能",target:()=>nav_view.logoButton()})
            return data
        }
    }
